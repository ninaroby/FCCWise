...
.controller('ViewController', function($scope, $http, $ionicModal, $state, $ionicHistory, $sce, GetData, FIREBASE_URL) {
    ...
    $scope.addTutor = function() {
        ...
        if (document.querySelector('#classes').value !== '') {
            ...
            for (i in $scope.tutor.short) {
                if (Object.hasOwnProperty.call($scope.tutor.short, i)) {
        ...
        if (document.querySelector('#courses').value !== '') {
            ...
            for (k in $scope.tutor.courses) {
                if (Object.hasOwnProperty.call($scope.tutor.courses, k)) {
                    ...
                    if ($scope.AllCourses !== -1) {
                        ...
                        AllMathCoursesUpTo = $scope.mathSections.indexOf($scope.tutor.courses[k].slice(0, -10));
                        $scope.tutor.AllCoursesUpTo = [];
                        for (z = 0; z <= AllMathCoursesUpTo; z++) {
    ...
    $scope.searchFunc = function(item) {
        var invalidSearchTerms = ['courses', 'firstName', 'hiring_department', 'lastName', 'schedule', 'details', 'weekday', 'location', 'times', 'short', 'subjects', 'thumbnail'], partIndex, result, query;
